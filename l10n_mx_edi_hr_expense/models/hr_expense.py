    ...
    def _compute_invoices_count(self):
        for sheet in self:
            ...
            sheet.l10n_mx_edi_invoices_count = len(invoices.filtered(
                lambda inv: inv.state in ('open', 'draft')))
        ...
        return {
            ...
            'domain': [('id', 'in', self.expense_line_ids.mapped(
                'l10n_mx_edi_invoice_id').filtered(
                    lambda inv: inv.state in ('open', 'draft')).ids)],
...
class HrExpense(models.Model):
    ...
    l10n_mx_edi_uuid = fields.Text(
        "UUID", track_visibility='onchange', copy=False,
        ...
        for expense in self:
            if not expense.sheet_id:
                ...
                expense.state = "refused"
                  ...
                  expense.l10n_mx_edi_invoice_id.state != 'paid'):
    ...
    def _force_create_super_employee(self):
        with Environment.manage():
            with registry(self.env.cr.dbname).cursor() as _cr:
                _env = api.Environment(_cr, self.env.uid, self.env.context)
 ...
 xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/">
   ...
   <ns1:Body>
    ...
    def functional9(self):
        ...
        return not bool(self.env['account.invoice'].sudo().search(
    ...
    def l10n_mx_edi_create_expense_invoice(self):
        ...
        for exp in expenses - to_omit:
            ...
            if label_draft in invoice.partner_id.category_id:
                continue
            invoice.with_context({}).action_invoice_open()
            exp.l10n_mx_edi_move_id = invoice.move_id.id
    ...
    def create_extra_line_local_taxes(self, tax):
        ...
        return{
            ...
            'name': tax['tax'].upper(),
            'quantity': 1,
            'uom_id': ieps.uom_id.id if ieps else self.product_uom_id.id,
    ...
    def l10n_mx_edi_get_invoice_data(self, inv):
        ...
        document_type = self.l10n_mx_edi_document_type
        journal = invoice.with_context(type=document_type)._default_journal()
        account = self.account_id.id or inv_line.with_context({
            'journal_id': journal.id,
            'type': document_type})._default_account()
        invoice_line_ids = []
        tax_global = self._get_tax_global()
        label = self.env.ref('l10n_mx_edi_hr_expense.tag_force_invoice_total')
        just_expense_products = self.env['ir.config_parameter'].sudo().get_param(
            'l10n_mx_edi_hr_expense_use_expense_products', False)
        for line in inv.get('lines', []):
            if not just_expense_products:
                no_id = line['no_id']
                self._cr.execute(
                    """SELECT pp.id
                ...
                res = product = self.product_id.id
            account = prod.browse(
        ...
        if inv.get('total_ieps'):
            ...
            invoice_line_ids.append((0, 0, {
                ...
                'quantity': 1,
                'uom_id': product_ieps.uom_id.id or False,
        ...
        acc_pay_term = (self.partner_id.property_supplier_payment_term_id
        ...
        return {
            'partner_id': self.partner_id.id,
            'reference': f'{inv["serie"]}{inv["number"]}',
    ...
    def l10n_mx_edi_get_tax(self, tax, product_id=False):
        ...
        if tax['type'] == 'ret':
            rate = rate * -1
        domain = [('tag_ids', 'in', tag_codes.get(tax['tax'])),
    ...
    def l10n_mx_edi_get_tax_group(self, taxes):
        ...
        tag_taxes = tax.browse(taxes).filtered(
            lambda tax: tag_tax in tax.tag_ids.ids)
        ...
        for group in grouped_taxes:
            if set(tag_taxes.ids).issubset(group.children_tax_ids.ids):
                return (group + tax.browse(taxes).filtered(
                    lambda tax: tag_tax not in tax.tag_ids.ids)).ids
    ...
    def _get_fuel_codes(self):
        ...
        fuel_codes.extend(self.company_id.l10n_mx_edi_fuel_code_sat_ids.mapped('code'))
        return fuel_codes
    ...
    def l10n_mx_edi_revert_expense(self):
        ...
        for record in self.filtered('sheet_id'):
            ...
            if invoice and invoice.state != 'cancel':
                ...
                invoice.mapped('move_id.line_ids').remove_move_reconcile()
                move_ids |= invoice.move_id
                invoice.action_cancel()
                invoice.write({
                    'move_name': False,
                    'reference': '%s-cancelled' % invoice.reference,
                })
            if not invoice and record.l10n_mx_edi_move_line_id:
                move = record.l10n_mx_edi_move_line_id.move_id
                move.button_cancel()
