...
class MxEdiAddendaBedBath(InvoiceTransactionCase):
    ...
    def test_001_addenda_in_xml(self):
        ...
        conf = self.env['res.config.settings'].create({
            'l10n_mx_addenda': 'bed_bath'})
        conf.install_addenda()
        self.partner_agrolait.write({
            'l10n_mx_edi_addenda': self.ref('l10n_mx_edi_addendas.bed_bath'),
            'ref': '7505000099632',
        })
        addenda_expected = misc.file_open(os.path.join(
            'l10n_mx_edi_addendas', 'tests', 'bed_bath_expected.xml')
        ).read().encode('UTF-8')
        ...
        invoice.partner_id.sudo().vat = 'VAU111017CG9'
        ...
        invoice.x_addenda_bed_bath = "DQ|2019-01-24|123456|123|10"
